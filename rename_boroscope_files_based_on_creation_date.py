...
for filerel in filelist:
    ...
    if ( extension.lower() in extensions ):
        if any(filename.startswith(prefix) for prefix in prefixes):
            total = total + 1
            files.append([filename,extension,file])
...
for filename, extension,file in files[:max_preview]:
    print(file)
...
should_continue = query_yes_no(f"total number of files: {total}, type yes to confirm")
...
for filename, extension,file in files:
    # Get the create time of the file, ONLY TESTED ON MAC
    ...
    if(os.path.isfile(newfilefull)):
        raise Exception(f"{newfilefull} already exists") 
